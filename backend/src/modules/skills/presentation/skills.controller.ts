...
export class SkillsController {
  ...
  @Get()
  @ApiOperation({
    summary: 'List the character’s skills with levels & progress',
  })
  ...
  heatmap(@CurrentUser('userId') userId: string, @Query('days') days?: string) {
    return this.skills.heatmap(userId, days ? Number(days) : undefined);
  }

  @Get(':key/history')
  @ApiOperation({ summary: 'Recent XP events for a single skill' })
  history(@CurrentUser('userId') userId: string, @Param('key') key: string) {
